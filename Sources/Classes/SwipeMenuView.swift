...
public struct SwipeMenuViewOptions {
    ...
    public struct TabView {
        ...
        /// isLanguageRTL set scroll direction. Defaults to `false`.
        public var isLanguageRTL : Bool  = false
    ...
    public var isLanguageRTL : Bool  = false {
        didSet {
            tabView.isLanguageRTL = isLanguageRTL
            contentScrollView.isLanguageRTL = isLanguageRTL
        }
    }
...
open class SwipeMenuView: UIView {
    ...
    private func setup(default defaultIndex: Int = 0) {
        ...
        contentScrollView?.updateSemantic()
...
extension SwipeMenuView: UIScrollViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if options.isLanguageRTL {
            if scrollView.contentOffset.x >= frame.width * CGFloat(((pageCount - 1) - currentIndex) + 1) {
                update(from: currentIndex, to: currentIndex - 1)
            } else if scrollView.contentOffset.x <= frame.width * CGFloat(((pageCount - 1) - currentIndex) - 1) {
                update(from: currentIndex, to: currentIndex + 1)
            }
        } else {
    ...
    private func moveAdditionView(scrollView: UIScrollView) {
        ...
        if let tabView = tabView, let contentScrollView = contentScrollView {
            ...
            if options.isLanguageRTL {
                ...
                case let offset where offset >= frame.width * CGFloat((pageCount - 1) - currentIndex):
